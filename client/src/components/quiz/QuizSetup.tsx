    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl transition-colors">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-950 p-1 rounded-xl mb-6 border border-gray-200 dark:border-gray-800">
          ...
          <button
            onClick={() => { setTab('custom'); setConfig({ ...config, questionCount: 'all', questionType: 'all', wordType: 'all' }); }}
        ...
        {tab === 'system' ? (
          ...
          <div className="mb-8">
            <CustomDataImport onDataImported={(data: any) => { 
              setCustomWords(data.vocabularies); 
              setError(null); 
            }} />
            {customWords.length > 0 && (
              <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-center justify-between">
                <span className="text-sm text-green-700 dark:text-green-400 font-medium">✅ Đã tạo danh sách {customWords.length} từ vựng</span>
            ...
            {error && tab === 'custom' && customWords.length === 0 && <p className="text-red-500 dark:text-red-400 text-sm mt-3">{error}</p>}
          </div>
        )}

        {(tab === 'system' || customWords.length > 0) && (
          <>
            {/* 2.5. Từ loại */}
            {tab === 'system' && (
              <div className="mb-6">
